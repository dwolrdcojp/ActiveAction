class DowntimesController < ApplicationController
  ...
  def index
    ...
    @downtime_pareto_by_min = Downtime.filter(params.slice(:date, :shift, :area, :line, :equipment, :reason_code)).group(:reason_code).sum(:downtime).sort
    @downtime_pareto_by_count = Downtime.filter(params.slice(:date, :shift, :area, :line, :equipment, :reason_code)).group(:reason_code).count.sort
  end
